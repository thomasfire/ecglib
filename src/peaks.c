...
void printWarning(char *message){
	if (GLOBAL_DEBUG) dprint("VERBOSE - WARING: %ld %ld \t", GLOBAL_COUNT, Rpeak);
    dprint("***WARNING: %s\n", message);
}
...
void printStatus(int RR, int Rpeak){
	...
	if (GLOBAL_DEBUG) dprint("VERBOSE - STATUS: %ld %ld \t", GLOBAL_COUNT, Rpeak);
    dprint("time: %ld   \t||\tpulse: <3 %0.2f\n", time, pulse);
    if (pulse < GLOBAL_STATS.pulse_min || GLOBAL_STATS.pulse_min == 0) {
        GLOBAL_STATS.pulse_min = pulse;
    }
    if (pulse > GLOBAL_STATS.pulse_max || GLOBAL_STATS.pulse_max == 0) {
        GLOBAL_STATS.pulse_max = pulse;
    }
}
